...
export function AdminDashboard({
  ...
  useEffect(() => {
    if (view === "transfers") void loadTransfers();
  }, [loadTransfers, view]);

  useEffect(() => {
    if (view === "content") void loadSharedWords();
  }, [loadSharedWords, view]);

  useEffect(() => {
    if (view === "content") void loadWordMediaOrphans();
  }, [loadWordMediaOrphans, view]);
                  ...
                  <div key={word.slug} className="border theme-border rounded-md p-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <p className="font-mono text-micro theme-faint truncate">
                          {word.activeShareCount} active · {word.pinProtectedCount} pin · next
                          expires {formatDate(word.nextExpiryAt)}
                        </p>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <Link
