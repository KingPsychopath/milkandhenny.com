...
export function CommunicationsPanel({ authFetch, onError, onStatus }: { authFetch: AuthFetch; onError: (message: string) => void; onStatus: (message: string) => void }) {
  ...
  return <div className="space-y-10">
    ...
    <section aria-labelledby="compose-heading" className="border-b theme-border pb-8">
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2"><label className="block"><span className="font-mono text-micro theme-muted">message type</span><AppSelect value={kind} onValueChange={(value) => setKind(value as Kind)} options={Object.entries(KIND_LABELS).map(([value, label]) => ({ value, label }))} tone="theme" variant="field" ariaLabel="Message type" className="mt-2" /></label><label className="block"><span className="font-mono text-micro theme-muted">audience</span><AppSelect value={audience} onValueChange={(value) => setAudience(value as Audience)} options={audienceOptions.map(([value, label]) => ({ value, label }))} tone="theme" variant="field" ariaLabel="Audience" className="mt-2" /></label></div>
      {kind === "event_update" ? <label className="mt-4 block"><span className="font-mono text-micro theme-muted">event</span><AppSelect value={eventSlug} onValueChange={setEventSlug} options={[{ value: "", label: "choose an event" }, ...events.map((event) => ({ value: event.slug, label: `${event.title} · ${dateLabel(event.startsAt)}` }))]} tone="theme" variant="field" ariaLabel="Event" className="mt-2" /></label> : null}
      ...
      <div className="mt-4 space-y-4"><Field label="subject" value={subject} onChange={setSubject} /><label className="block"><span className="font-mono text-micro theme-muted">message</span><textarea value={body} onChange={(event) => setBody(event.target.value)} rows={8} placeholder="A short note with one clear reason to click." className="mt-1 w-full rounded border theme-border bg-transparent px-3 py-2 font-mono text-sm leading-relaxed focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--prose-hashtag)]" /><span className="mt-1 block font-mono text-micro theme-faint">Plain text. Blank lines create paragraphs.</span></label>
        <div className="grid gap-3 md:grid-cols-[8rem_1fr]"><label className="block"><span className="font-mono text-micro theme-muted">media type</span><AppSelect value={mediaKind} onValueChange={(value) => setMediaKind(value as typeof mediaKind)} options={[{ value: "image", label: "image" }, { value: "gif", label: "GIF" }, { value: "video", label: "video link" }]} tone="theme" variant="field" ariaLabel="Media type" className="mt-2" /></label><Field label="media URL" value={mediaUrl} onChange={setMediaUrl} hint="Public image or GIF URL. Video emails use a linked poster." /></div>
