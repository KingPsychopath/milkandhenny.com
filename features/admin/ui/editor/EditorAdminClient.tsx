...
import { WordCreateForm } from "./components/WordCreateForm";
import { WordEditSection } from "./components/WordEditSection";
import { WordShareSection } from "./components/WordShareSection";
...
export function EditorAdminClient() {
  ...
  const handleAppendSnippet = useCallback(
    (snippet: string) => {
  ...
  const saveWordToApi = useCallback(async () => {
    ...
    try {
      ...
      const data = (await res.json().catch(() => ({}))) as NoteRecord & {
        error?: string;
        conflict?: boolean;
  ...
  const requestAutosave = useCallback(
    ...
    [clearAutosaveTimer, editPayload, hasLoadedSelected, isEditDirty, selectedSlug],
  );
    ...
    try {
      ...
      const res = await fetch(
        `/api/words/${encodeURIComponent(link.slug)}/shares/${encodeURIComponent(link.id)}`,
        {
          ...
          body: JSON.stringify(
            isExpired
              ? { rotateToken: true, expiresInDays: newShareExpiryDays }
              : { rotateToken: true },
          ),
        },
      );
