...
export function WordEditSection({
  ...
  const prefixSelectedLines = (getPrefix: (index: number) => string) => {
    updateMarkdownWithSelection((value, start, end) => {
    ...
    <div className="border theme-border rounded-md p-4 space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="space-y-1">
          <h2 className="font-mono text-xs theme-muted">
            edit ·{" "}
            <Link
              to={wordPathForVisibility(selected.slug, selected.visibility)}
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
        ...
        >
          <div
            className={
              editorFocusMode ? "max-w-5xl mx-auto h-full flex flex-col gap-3" : "space-y-3"
            }
          >
            {!showPreview || editorFocusMode ? (
              <div
                className={`flex flex-wrap items-center justify-between gap-2 ${
                  editorFocusMode
                    ? "sticky top-0 z-10 bg-[var(--background)]/95 backdrop-blur py-1"
                    : ""
                }`}
              >
