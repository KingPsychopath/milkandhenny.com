...
function SingleVisualContent({
  ...
  if (file.previewStatus !== "ready" && !canRenderOriginalVisual(file)) {
    ...
    return (
      <div
        className="flex flex-col items-center justify-center gap-3 py-20 px-8"
        onClick={(e) => e.stopPropagation()}
      >
        <svg
          width="40"
...
function MultiVisualContent({
  ...
  return (
    <div
      className="flex flex-col items-center gap-4 pb-4 sm:pb-0"
      onClick={(e) => e.stopPropagation()}
    >
      <div className="flex items-center gap-2 font-mono text-micro tracking-wide">
        <span className="text-white/35">
          {item.type === "live_photo" ? "live photo" : "raw pair"}
        </span>
        <button
          type="button"
...
export function TransferGallery({ transferId, files, groups, deleteToken }: TransferGalleryProps) {
  ...
  const [pendingMultipartDownload, setPendingMultipartDownload] =
    useState<PendingMultipartDownload | null>(null);
  ...
  const [selectedTimeParam, setSelectedTimeParam] = useState<string | null>(() =>
  ...
  useEffect(() => {
    ...
    try {
      ...
      const restored = parsed.filter(
        (value): value is string => typeof value === "string" && allowedIds.has(value),
      );
  ...
  const scopedEntries = useMemo(() => {
    const separateVisualItems = grouped.visual.map(
      (file) =>
        ({ id: `single:${file.id}`, type: "single", file }) as const satisfies VisualGalleryItem,
    );
    ...
    return [
      ...visualItems.map(
        (item) => ({ id: item.id, type: "visual", item }) as const satisfies GalleryEntry,
      ),
      ...fileEntries.map(
        (file) => ({ id: `file:${file.id}`, type: "file", file }) as const satisfies GalleryEntry,
      ),
    ];
  ...
  const pageSections = useMemo(() => {
    ...
    pageEntries,
    pageStartIndex,
    timeFilterCategoryById,
  ]);

  useEffect(() => {
    setVisibleVisualCount((prev) =>
      Math.min(visualItems.length, Math.max(prev, INITIAL_VISUAL_RENDER_COUNT)),
    );
  }, [visualItems.length]);
    ...
    () =>
      filteredEntries.reduce((sum, entry) => {
        if (entry.type === "visual")
          return sum + Number(isVisualItemSelected(entry.item, selectedIds));
  ...
  const allFilteredSelected =
    filteredEntries.length > 0 && selectedInFilteredCount === filteredEntries.length;
  ...
  const lightboxVisualFiles = useMemo(
    () =>
      filteredEntries
        .filter(
          (entry): entry is Extract<GalleryEntry, { type: "visual" }> => entry.type === "visual",
        )
  ...
  const goNext = useCallback(() => {
    ...
    setLightboxIndex((prev) =>
      prev !== null && prev < lightboxVisualFiles.length - 1 ? prev + 1 : prev,
    );
  ...
  useEffect(() => {
    ...
    const candidates = [
      lightboxVisualFiles[lightboxIndex - 1],
      lightboxVisualFiles[lightboxIndex + 1],
    ].filter((item): item is VisualGalleryItem => !!item);
    ...
    async (file: TransferFileData) => {
      ...
      try {
        const res = await fetch(
          `/api/transfers/${transferId}/files/${encodeURIComponent(file.id)}`,
          {
            method: "DELETE",
  ...
  ).filter((tab) => tab.key === "all" || (tab.count > 0 && tab.count < filterCounts.all));

  const downloadLabel = preparingDownload
    ? "[ preparing download... ]"
  ...
  const renderSection = (section: GallerySection) => {
    ...
    return (
      <section
        key={section.key}
        className={section.title ? "mt-8 first:mt-0" : ""}
  ...
  return (
    <div>
      ...
      {pendingMultipartDownload ? (
        <div className="mb-4 rounded-sm border theme-border px-3 py-3">
          <p className="font-mono text-micro tracking-wide">
            This download will be split into {pendingMultipartDownload.plan.partCount} parts (up to{" "}
            {formatBytes(BLOB_ZIP_DOWNLOAD_LIMIT_BYTES)} each).
          </p>
      ...
      streamingNoticeBytes >= LARGE_STREAMING_ZIP_NOTICE_BYTES ? (
        <p className="mb-4 font-mono text-nano theme-muted tracking-wide">
          Large ZIPs in Chrome or Edge may temporarily require about 2x the archive size free on
          disk before the file is committed.
        </p>
      ) : null}
      {deleteError ? (
      ...
      {(visualItems.length > 0 || nonVisualFiles.length > 0) && (
        <>
          ...
          <div className="space-y-8">{pageSections.map((section) => renderSection(section))}</div>
        </>
      )}
...
function FileCard({
  ...
  const checkbox = (
    <SelectionToggle selected={isSelected} onToggle={onToggleSelect} variant="surface" />
  );
