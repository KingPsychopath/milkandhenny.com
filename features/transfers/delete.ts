...
function getTransferFileDeleteKeys(
  ...
  return Array.from(
    new Set(
      [file.storageKey, file.originalStorageKey, expected.thumbKey, expected.fullKey].filter(
...
function resolveTransferFileForDelete<T extends Pick<TransferFile, "id" | "filename">>(
  files: readonly T[],
  selector: string,
): T | null {
  ...
  if (exactFilename.length > 1) {
    throw new Error(`Multiple files match filename "${selector}". Use the file id instead.`);
  }
