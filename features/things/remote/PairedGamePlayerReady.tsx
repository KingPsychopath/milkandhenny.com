...
export function PairedGamePlayerReady({
  ...
  return (
    <div className="things-game things-game--night text-white">
      ...
      >
        <p className="font-mono text-micro uppercase tracking-[0.2em] text-white/45">{gameName}</p>
        ...
        <p className="mx-auto mt-3 max-w-sm font-serif text-lg leading-relaxed text-white/55">
          {detail}
        </p>
        {onFullscreen ? (
          <button
            ...
            className="mt-6 min-h-11 font-mono text-xs text-white/60"
          >
            use full screen
          </button>
        ) : null}
        <button
          type="button"
          onClick={onStart}
          className="mt-4 min-h-16 rounded-full bg-[var(--things-amber)] px-6 font-mono text-sm font-bold text-black"
        >
          start
        </button>
      </main>
      {leaveConfirmationOpen ? (
        <EndGameDialog
          ...
          description="This closes the connection for both phones. You can ask the judge for a new invite later."
          cancelLabel="stay"
