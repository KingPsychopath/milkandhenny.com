...
export function remoteGameSetup(value: unknown): RemoteGameSetup {
  ...
  const timerSeconds =
    typeof data.timerSeconds === "number" && Number.isInteger(data.timerSeconds)
      ? Math.max(0, Math.min(60, data.timerSeconds))
      : 30;
  const roundWordCount =
    typeof data.roundWordCount === "number" && Number.isInteger(data.roundWordCount)
...
function command(value: unknown): RemoteCommandRequest {
  ...
  const createdAt =
    typeof data.createdAt === "number" && Number.isFinite(data.createdAt) ? data.createdAt : NaN;
  ...
  if (data.type === "amend") {
    ...
    return {
      id,
      type: "amend",
      resultId: shortText(data.resultId, 80),
      decision,
      createdAt,
      ...target,
    };
    ...
    secondsRemaining:
      data.secondsRemaining === null
        ...
        : typeof data.secondsRemaining === "number" &&
            ...
            data.secondsRemaining <= 3600
          ? data.secondsRemaining
          : null,
    decisionClosesAt:
      typeof data.decisionClosesAt === "number" && Number.isFinite(data.decisionClosesAt)
        ? data.decisionClosesAt
        : undefined,
    decisionGraceEndsAt:
      typeof data.decisionGraceEndsAt === "number" && Number.isFinite(data.decisionGraceEndsAt)
        ? data.decisionGraceEndsAt
        : undefined,
...
function syncedSnapshot(value: unknown): RemoteSyncedSnapshot {
  ...
  if (!Array.isArray(data.commandReceipts) || data.commandReceipts.length > 20)
  ...
  return {
    ...
    commandReceipts: data.commandReceipts.map((value) => {
      ...
      return {
        ...
        sequence:
          typeof receipt.sequence === "number" ? Math.max(0, Math.floor(receipt.sequence)) : 0,
...
export const syncPairedGamePlayerFn = createServerFn({ method: "POST" })
  .validator((value: unknown) => {
    ...
    return {
      ...
      playerToken: token(data.playerToken),
      snapshot: syncedSnapshot(data.snapshot),
      lastCommandSequence:
        typeof data.lastCommandSequence === "number"
          ? Math.max(0, Math.floor(data.lastCommandSequence))
          : 0,
    };
  })
...
export const readPairedGameJudgeFn = createServerFn({ method: "POST" })
  .validator((value: unknown) => {
    ...
    return {
      roomId: roomId(data.roomId),
      judgeToken: token(data.judgeToken),
      judgeEpoch: shortText(data.judgeEpoch, 80),
