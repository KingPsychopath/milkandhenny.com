    ...
    >
      <header className="spelling-play-header grid grid-cols-3 items-center px-5 py-4">
        <span className="flex items-center gap-3">
          <button
            type="button"
            onClick={onEnd}
            className="min-h-11 rounded-full border border-black/20 px-3 font-mono text-xs"
          >
            end round
          </button>
          {remoteBadge}
        </span>
        <span className="justify-self-center rounded-full border border-black/15 px-4 py-2 font-mono text-lg font-semibold tabular-nums">
        ...
        {paused ? (
          <div role="alert">
            <p className="font-mono text-micro uppercase tracking-[0.2em] text-black/50">
              {pauseReason ? "motion paused · timer stopped" : "word paused · timer stopped"}
            </p>
            <h1 className="mt-4 font-serif text-5xl font-semibold">
              {pauseReason === "wrong-orientation"
                ? "Return the phone to its starting orientation."
            ...
            <div className="mt-7 flex flex-col items-center gap-2">
              {pauseReason ? (
                <p className="font-mono text-xs text-black/55">the round resumes automatically</p>
              ) : (
                <button
                  type="button"
                  onClick={onResume}
                  className="min-h-12 rounded-full bg-black px-6 font-mono text-sm font-semibold text-white"
                >
                  resume with {seconds === null ? "no timer" : `${seconds}s left`}
                </button>
              )}
              <button
                type="button"
                onClick={onEnd}
                className="min-h-11 px-4 font-mono text-xs text-black/55 underline underline-offset-4"
              >
                end round
              </button>
            </div>
          ...
          <>
            <p className="font-mono text-micro uppercase tracking-[0.2em] text-black/45">
              {presenting
                ? "listen"
                : awaitingRemoteDecision
                  ? "time’s up · checking judge"
                  : feedback === "timed_out"
                    ? "timed out"
                    : (feedback ?? "spell this")}
            </p>
            <HighlightedWord
              ...
              listening={listening}
            />
            {item.definition || item.partOfSpeech ? (
              <p className="spelling-play-definition mt-5 max-w-lg font-serif text-lg leading-relaxed text-black/60">
                {item.partOfSpeech ? <em>{item.partOfSpeech}</em> : null}
                {item.partOfSpeech && item.definition ? " · " : null}
                {item.definition}
              </p>
            ) : null}
            ...
            */
            ...
            {followingEnabled ? (
              <div
                className="mt-5 flex min-h-11 flex-wrap items-center justify-center gap-x-3 gap-y-1 font-mono text-micro text-black/55"
                role="status"
                aria-label={
                  listening
                    ? "Microphone on and listening"
                    : (followingError ?? "Spelling follower ready")
                }
              >
                <span
                  aria-hidden="true"
                  className={listening ? "text-emerald-800" : "text-black/35"}
                >
                  ●
                </span>
                <span>
                ...
                {listening ? (
                  <span
                    aria-hidden="true"
                    className="h-1.5 w-16 overflow-hidden rounded-full bg-black/10"
                  >
                    <span
                      className="block h-full origin-left rounded-full bg-emerald-800 transition-transform duration-75"
                      style={{ transform: `scaleX(${Math.max(0.06, inputLevel)})` }}
                    />
                  </span>
                ) : null}
                {followingError ? (
            ...
            {transcript ? (
              <p className="mt-5 font-mono text-sm tracking-[0.14em] text-black/65">
                heard · {transcript}
              </p>
            ) : null}
            <div className="spelling-play-actions mt-8 flex flex-wrap justify-center gap-2">
              <button
                type="button"
                onClick={() => onReplay()}
      ...
      <footer className="spelling-play-footer grid grid-cols-2 gap-3 p-4 pb-[max(1rem,env(safe-area-inset-bottom))] landscape:grid-cols-3">
        <button
          ...
          className="min-h-14 rounded-full border border-black/20 bg-black/5 font-mono text-sm font-semibold disabled:opacity-35"
        >
          incorrect{tiltEnabled ? " ↑" : ""}
        </button>
        <button
          type="button"
          disabled={paused || presenting || controlsLocked}
        ...
        <button
          ...
          className="col-span-2 min-h-11 rounded-full font-mono text-xs text-black/55 underline underline-offset-4 disabled:opacity-35 landscape:col-span-1 landscape:min-h-14"
        >
          skip this word
        </button>
      </footer>
...
function HighlightedWord({
  ...
  return (
    <h1
      aria-label={word}
      className="spelling-play-word mt-4 max-w-3xl break-words font-serif text-5xl font-semibold leading-none tracking-tight sm:text-7xl"
    >
