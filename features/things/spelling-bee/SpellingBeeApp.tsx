...
import { cancelLocalSpeech, speakWord } from "../spelling/localSpeech";
...
function SpellingBeeExperience({ remoteSession }: { remoteSession?: RemotePlayerSession }) {
  ...
  const undoDecision = useCallback(() => {
    ...
    if (wordAlreadyAdvanced) setWordIndex((current) => Math.max(0, current - 1));
    setFeedback(null);
    setWordState({ status: "idle" });
    processing.current = false;
  ...
  if (joinedDeck) return <PairedGamePlayerReady gameName="Spelling Bee" deckName={joinedDeck.name} detail={`Words will be read aloud on this phone${timerSeconds ? ` with ${timerSeconds} seconds per word` : " with no timer"}.`} judgeConnected={remote.judgeConnected} onLeave={remote.closeRoom} onStart={() => void startRound(joinedDeck)} />;
