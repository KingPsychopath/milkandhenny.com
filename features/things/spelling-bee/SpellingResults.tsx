...
export function SpellingResults({
  results,
  onBack,
  onAgain,
  onAmend,
  ...
  onBack: () => void;
  onAgain: () => void;
  onAmend: (id: string, decision: SpellingResult["decision"]) => void;
}) {
    ...
    <div className="things-game things-game--cream text-black">
      ...
      <main id="main" className="mx-auto w-full max-w-lg flex-1 px-6 pb-10 text-center">
        ...
        <button
          type="button"
          onClick={onAgain}
          className="mt-7 min-h-14 w-full rounded-full bg-black font-mono text-sm font-semibold text-white"
        >
          another round
        </button>
        <section className="mt-10 text-left" aria-labelledby="spelling-results">
          <h2
            id="spelling-results"
            className="font-mono text-micro uppercase tracking-[0.18em] text-black/50"
          >
            the words · tap to correct
          </h2>
          <ul className="mt-3 border-t border-black/15">
            {results.map((result) => (
              <li
                key={result.id}
                className="grid grid-cols-[1fr_auto] items-center gap-4 border-b border-black/15 py-3"
              >
                <span className="font-serif text-lg">{result.word}</span>
                <AppSelect
                  ariaLabel={`Change result for ${result.word}`}
                  value={result.decision}
                  onValueChange={(value) => onAmend(result.id, value as SpellingResult["decision"])}
                  tone="cream"
                  options={[
