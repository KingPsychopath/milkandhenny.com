...
const GROUPS: LiarsWordGroup[] = [
  ...
  /*
   * Split out of a single "a person or an animal" category, which was two categories wearing one
   * coat. A twelve-word board drawn from it came out as eleven jobs and one `sheepdog`, and a lone
   * animal among people is a free read for everybody: the crew holding it cannot say anything
   * person-shaped, and the imposter can see the odd one out from across the table.
   *
   * The same pass dropped four pairs that were neither — `moustache/beard`, `veterinary/paediatric`,
   * `vampire/ghost` and `fishmonger/deli counter`.
   */
  {
    category: "somebody's job",
    pairs: [
      ["pirate", "viking"],
      ["dentist", "barber"],
