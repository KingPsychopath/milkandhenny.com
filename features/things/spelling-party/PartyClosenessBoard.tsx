...
export function PartyClosenessBoard({
  answers,
  currentPlayerId,
}: {
    ...
    <ol className="mt-7 space-y-3" aria-label="Spellings ranked by closeness">
      {answers.map((answer) => {
        ...
        return (
          <li
            key={answer.playerId}
            aria-label={`${ordinal(answer.place)} place, ${answer.name}, ${label}, spelled ${answer.answer || "nothing"}`}
            className={`rounded-2xl border p-4 text-left ${answer.playerId === currentPlayerId ? "border-amber-200/45 bg-amber-200/[0.07]" : "border-white/12 bg-white/[0.03]"}`}
          >
            <div className="grid grid-cols-[2.5rem_1fr_auto] items-baseline gap-3">
              ...
              <span className="min-w-0 truncate font-serif text-lg">
                {answer.name}
                {answer.playerId === currentPlayerId ? " · you" : ""}
              </span>
              <span
                className={`font-mono text-micro ${answer.correct ? "text-amber-200" : "text-white/45"}`}
              >
                {label}
              </span>
            </div>
            <div className="mt-2 grid grid-cols-[2.5rem_1fr] items-center gap-3">
              ...
              <div>
                <p className="truncate font-mono text-sm tracking-[0.08em] text-white/75">
                  {answer.answer || "—"}
                </p>
                <div
                  aria-hidden="true"
                ...
                >
                  <div
                    className={`h-full origin-left rounded-full transition-[width] duration-500 motion-reduce:transition-none ${answer.correct ? "bg-[var(--things-amber)]" : "bg-white/45"}`}
                    style={{ width: `${width}%` }}
                  />
                </div>
              </div>
