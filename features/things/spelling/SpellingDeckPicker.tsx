    ...
    <section className="mx-auto mt-10 max-w-lg" aria-labelledby="spelling-decks">
      <div className="flex min-h-11 items-center justify-between gap-4">
        ...
        <button
          ...
          className="min-h-11 rounded-full border border-white/15 px-4 font-mono text-xs"
        >
          + make a deck
        </button>
      </div>
      <div className="mt-3 grid gap-3">
        {decks.map((deck) => {
          ...
          return (
            <div
              key={deck.id}
              className={`grid grid-cols-[1fr_auto] items-center gap-2 rounded-3xl border p-2 ${selected ? "border-white/60 bg-white/12" : "border-white/12 bg-white/[0.04]"}`}
            >
              <button
                type="button"
                onClick={() => onSelectDeck(deck.id)}
                aria-pressed={selected}
                className="grid min-h-20 grid-cols-[2.75rem_1fr_auto] items-center gap-3 p-2 text-left"
              >
                <span aria-hidden="true" className="font-serif text-2xl text-white/60">
                  {deck.symbol}
                </span>
                <span>
              ...
              {customDeckIds.has(deck.id) ? (
                <button
                  type="button"
                  onClick={() => onEditDeck(deck.id)}
                  className="min-h-11 rounded-full px-3 font-mono text-xs text-white/55"
                >
                  edit
                </button>
              ) : null}
