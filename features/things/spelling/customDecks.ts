...
export function parseSpellingWords(value: string) {
  ...
  for (const rawLine of value.split(/\r?\n/)) {
    ...
    if (!line) continue;
    const [rawWord, partOfSpeech, definition, speakAs, sentence] = line
      .split("|")
      .map((part) => part.trim());
...
function normalise(value: unknown): CustomSpellingDeck[] {
  ...
  return value.flatMap((item): CustomSpellingDeck[] => {
    ...
    if (typeof deck.id !== "string" || typeof deck.name !== "string" || !Array.isArray(deck.words))
      return [];
    const words = deck.words
      .flatMap((candidate): SpellingWord[] => {
        ...
        return [
          {
            ...
            partOfSpeech:
              typeof entry.partOfSpeech === "string" ? entry.partOfSpeech.slice(0, 30) : undefined,
            definition:
              typeof entry.definition === "string" ? entry.definition.slice(0, 220) : undefined,
...
export function createCustomSpellingDeck(
  name: string,
  words: SpellingWord[],
  id?: string,
): CustomSpellingDeck {
  return {
    id: id ?? `spelling-custom-${crypto.randomUUID()}`,
    name: name.trim().slice(0, 50) || "My words",
