  ...
  return (
    <div className="things-game things-game--night text-white">
      ...
      <main id="main" className="mx-auto w-full max-w-lg flex-1 px-5 pb-10">
        ...
        <label htmlFor="spelling-deck-name" className="mt-8 block font-mono text-xs text-white/65">
          deck name
        </label>
        <input
          id="spelling-deck-name"
          value={name}
          onChange={(event) => setName(event.target.value)}
          maxLength={50}
          className="mt-2 min-h-14 w-full rounded-2xl border border-white/15 bg-white/[0.06] px-4 font-serif text-lg"
          placeholder="Family spelling night"
        />
        <label htmlFor="spelling-words" className="mt-6 block font-mono text-xs text-white/65">
          words · {words.length}
        </label>
        <textarea
          id="spelling-words"
        ...
        />
        <p
          id="spelling-word-format"
          className="mt-2 font-mono text-micro leading-relaxed text-white/40"
        >
          word | part of speech | short definition | pronounce as | sentence clue
        </p>
        {words[0] ? (
          <button
            type="button"
            onClick={() =>
              void speakWord(words[0]).then((ok) =>
                setMessage(
                  ok ? `Previewed “${words[0].word}”.` : "No local English voice is installed.",
                ),
              )
            }
            className="mt-3 min-h-11 rounded-full border border-white/15 px-4 font-mono text-xs text-white/65"
          >
            preview first word
          </button>
        ) : null}
        <p
          id="spelling-builder-message"
          aria-live="polite"
          className="mt-2 min-h-4 font-mono text-xs text-white/55"
        >
          {message}
        </p>
        <button
          type="button"
          disabled={!canSave}
          onClick={() => onSave(createCustomSpellingDeck(name, words, deck?.id))}
        ...
        {deck ? (
          <button
            ...
            className="mt-4 min-h-11 w-full font-mono text-xs text-white/45"
          >
            delete this deck
          </button>
        ) : null}
      ...
      {deck && confirmingDelete ? (
        <GameActionDialog
          ...
          description="This removes it from both Spelling Bee modes on this device."
