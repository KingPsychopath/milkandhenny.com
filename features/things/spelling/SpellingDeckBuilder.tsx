...
export function SpellingDeckBuilder({
  ...
  return (
    <div className="things-game things-game--night text-white">
      ...
      {deck && confirmingDelete ? <GameActionDialog tone="dark" eyebrow="delete deck" title={`Delete “${deck.name}”?`} description="This removes it from both Spelling Bee modes on this device." cancelLabel="keep deck" confirmLabel="delete deck" onCancel={() => setConfirmingDelete(false)} onConfirm={() => onDelete(deck)} /> : null}
    </div>
  );
}
