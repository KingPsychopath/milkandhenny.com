  ...
  if (node.tagName === "ul") {
    return (
      node.children
        ...
        .map(
          (child) =>
            `- ${normalizeCellText(child.children?.map(stringifyMarkdownNode).join(" ") ?? "")}`,
        )
        .join("\n") ?? ""
    );
  }
  if (node.tagName === "ol") {
    return (
      node.children
        ...
        .map(
          (child, index) =>
            `${index + 1}. ${normalizeCellText(child.children?.map(stringifyMarkdownNode).join(" ") ?? "")}`,
        )
        .join("\n") ?? ""
    );
  }
...
function MarkdownIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false" className="prose-table-icon">
      <path
        d="M6 4h9l5 5v11a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2Z"
        fill="none"
        stroke="currentColor"
        strokeWidth="1.8"
        strokeLinejoin="round"
...
function PinIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false" className="prose-table-icon">
      <path
        d="M8 4h8l-1.5 5 3.5 3v1H6v-1l3.5-3L8 4Z"
        fill="none"
        stroke="currentColor"
        strokeWidth="1.8"
        strokeLinejoin="round"
...
export function WordBodyTable({
  ...
  const sourceTableData = React.useMemo(
    () => extractTableDataFromNode(node as MarkdownNode | undefined),
    [node],
  );
  ...
  React.useEffect(() => {
    ...
    const syncOverflow = () => {
      setOverflowState(
        getOverflowState(scrollEl.scrollLeft, scrollEl.clientWidth, scrollEl.scrollWidth),
      );
    };
  ...
  const getTableData = React.useCallback(() => {
    if (sourceTableData.headers.length > 0 || sourceTableData.rows.length > 0)
  ...
  return (
    <div
      className={`prose-table ${isCollapsed ? "prose-table--compact" : "prose-table--expanded"} ${isFirstColumnPinned ? "prose-table--pinned" : ""} ${overflowState.hasHorizontalOverflow ? "prose-table--scrollable" : ""}`}
    >
      <div
        ref={scrollRef}
...
export function WordBodyTableCell({
  ...
  return (
    ...
    >
      <div
        className={`prose-table-cell ${shouldClamp ? "prose-table-cell--clamped" : ""} ${isToggleable ? "prose-table-cell--toggleable" : ""}`}
      >
        {children}
      </div>
    </td>
