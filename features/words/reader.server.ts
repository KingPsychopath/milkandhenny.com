...
import { requireAuthFromServerContext } from "@/features/auth/context.server";
import { getCookie } from "@/lib/http/cookies";
import { verifyWordAccessToken, wordAccessCookieName } from "./share.server";
...
async function hasWordCookieAccessForSlug(slug: string): Promise<boolean> {
  const token = getCookie(getRequest(), wordAccessCookieName(slug));
