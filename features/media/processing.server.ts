...
async function safeReadOrientation(exifr: ExifrModule, raw: Buffer): Promise<number> {
  try {
...
async function safeParseMetadata(exifr: ExifrModule, raw: Buffer): Promise<ExifrParsedTags | null> {
  try {
 ...
 */
async function extractPreviewWithExiftool(raw: Buffer, sourceExt: string): Promise<Buffer> {
  ...
  return withTempFile("raw-preview", ext, raw, async (tempFile) => {
    ...
    for (const tag of tags) {
      try {
        const { stdout } = await execFileAsync("exiftool", ["-b", tag, tempFile], {
          encoding: "buffer",
    ...
    throw new RawPreviewUnavailableError(sourceExt, "no usable embedded preview found");
  });
}
...
async function processRawWithExiftool(
  ...
  const buffer = await sharp(rotated).jpeg({ quality: 95, mozjpeg: true }).toBuffer();
...
async function processToWebP(
  ...
  const outHeight = width > maxWidth ? Math.round(height * (maxWidth / width)) : height;
