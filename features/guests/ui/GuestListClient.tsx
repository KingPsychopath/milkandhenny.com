...
export function GuestListClient({ initialGuests }: GuestListClientProps) {
  ...
  const votingSecondsRemaining =
    isVotingOpen && voteWindowOpenUntil
  ...
  const handlePrintVoteSheet = async () => {
    if (voteCodeWords === 1 && sheetCount > 50) {
      setVoteWindowStatus(
        "1-word codes collide quickly in large sheets. Switch to 2 words or print 50 or less.",
      );
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-stone-100">
      ...
      >
        <div className="font-mono">
          ...
          <p className="text-micro text-stone-500 mt-1">
            ttl {formatMinutesShort(codeTtlMinutes)}{" "}
            {sheetExpiresAt ? `• expires ${new Date(sheetExpiresAt).toLocaleTimeString()}` : ""}
          </p>
        </div>
        ...
        <div className="bd-print-grid mt-3 grid grid-cols-3 gap-3">
          {sheetRows.map((r) => (
            <div
              key={r.code}
              className="bd-print-card border border-stone-200 rounded-lg p-3 text-center"
            >
              <img
                src={r.qr}
                alt={`QR ${r.code}`}
              ...
              />
              <p className="bd-print-code mt-2 font-mono text-sm tracking-wider text-stone-900">
                {r.code}
              </p>
      ...
      <main
        id="main"
        className="max-w-lg mx-auto bg-white min-h-screen shadow-xl shadow-stone-300/50"
      >
        {/* Header */}
        ...
        <section className="mx-4 mt-4 p-4 bg-stone-50 border border-stone-200 rounded-xl">
          <div className="flex items-center justify-between gap-3">
            ...
            <Link
              to="/best-dressed"
              className="text-stone-500 hover:text-amber-700 text-sm transition-colors"
            >
              open →
            </Link>
          </div>

          <p className="mt-2 font-mono text-micro text-stone-400">
            tap the buttons; options are optional
          </p>
            ...
            <div className="mt-3 p-3 rounded-lg bg-white border border-stone-200">
              ...
              {voteCodeExpiry ? (
                <p className="text-stone-400 text-xs mt-1">
                  expires {new Date(voteCodeExpiry).toLocaleTimeString()}
                </p>
              ) : null}
              {voteCodeQr ? (
