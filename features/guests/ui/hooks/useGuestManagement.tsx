...
export function useGuestManagement({
  ...
  async function handleBootstrap() {
    ...
    try {
      const step = await ensureStepUpToken();
      if (!step) return;
      const res = await authFetch("/api/admin/guests/bootstrap", {
        method: "POST",
        headers: withStepUpHeaders(step),
      });
      const payload = await readResponsePayload(res);
      ...
      flashSuccess(
        data.bootstrapped
          ? `Loaded ${(data.count as number) ?? 0} guests from CSV`
          : (data.message as string) || "Guests already exist",
      );
      onCSVImported();
    } catch {
  ...
  async function handlePartyReset() {
    ...
    try {
      ...
      const voteRes = await authFetch("/api/best-dressed", {
        method: "DELETE",
        headers: withStepUpHeaders(step),
      });
      if (!voteRes.ok) {
      ...
      flashSuccess(
        `Party ready! Loaded ${(guestData.count as number) ?? 0} guests, all votes cleared.`,
        5000,
      );
      onCSVImported();
    } catch {
  ...
  async function fetchBestDressedData() {
    try {
      ...
      const data = (payload.json ?? {}) as Record<string, unknown>;
      setBestDressedLeaderboard(
        (Array.isArray(data.leaderboard) ? data.leaderboard : []) as LeaderboardEntry[],
      );
  ...
  async function handleWipeBestDressed() {
    ...
    try {
      const step = await ensureStepUpToken();
      if (!step) return;
      const res = await authFetch("/api/best-dressed", {
        method: "DELETE",
        headers: withStepUpHeaders(step),
      });
      if (res.ok) {
