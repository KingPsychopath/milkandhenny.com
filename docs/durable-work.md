...
Postgres is the default for a future generic durable job when no data-local transaction requires
...
## Effect `PersistedQueue`

The unstable `PersistedQueue` API is only a candidate for future generic Effect-owned jobs. Adoption
requires an idempotent handler, acceptance of at-least-once delivery, operational access to failed
jobs, a migration and transaction-participation design, and compatibility with the application's
existing Redis or Postgres adapters. It must not become a `DefaultQueue` or replace the specialized
email, media, or official-result mechanisms without proving equivalent guarantees.
