...
describe("media download helpers", () => {
  ...
  it("reads content-length from a HEAD response", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(
        async () => new Response(null, { status: 200, headers: { "content-length": "12345" } }),
      ),
    );
  ...
  it("prefers worker zip fallback only for non-picker multi-file downloads when configured", async () => {
    ...
    vi.stubEnv("VITE_MULTI_FILE_ZIP_MODE", "auto");
  ...
  it("surfaces text error bodies from download presign responses", async () => {
    ...
    await expect(
      getPresignedDownloadUrl("transfers/demo/originals/file.jpg", "file.jpg"),
    ).rejects.toThrow("An error occurred while preparing the download.");
  });
