...
describe("Party Typing rooms", () => {
  it("uses a validated room-local custom deck without exposing its word to players", async () => {
      ...
      words: [
        {
          id: "custom-one",
          word: "kumquat",
          definition: "a small citrus fruit",
          sentence: "She sliced a kumquat into the salad.",
        },
    ...
    const started = await applyPresenterAction({
      ...
      action: { actionId: "custom-start", type: "round.start" },
    });
    ...
    const playerView = await readPartySnapshot({
      roomId: room.roomId,
      role: "player",
      credential: player.playerToken,
      playerId: player.playerId,
      lastSequence: 0,
    });
    expect(playerView.snapshot?.round).not.toHaveProperty("spokenWord");
    expect(JSON.stringify(playerView.snapshot)).not.toContain(
      started.snapshot?.round?.spokenWord ?? "missing",
    );
    ...
    await applyPlayerAction({
      ...
      action: {
        actionId: "custom-repeat",
        type: "clue.request",
        roundId: started.snapshot!.round!.roundId,
        clue: "repeat",
      },
    });
    const presenterView = await readPartySnapshot({
      roomId: room.roomId,
      role: "presenter",
      credential: room.presenterToken,
      lastSequence: 0,
    });
    const playerAfterClue = await readPartySnapshot({
      roomId: room.roomId,
      role: "player",
      credential: player.playerToken,
      playerId: player.playerId,
    ...
    expect(presenterView.snapshot?.round?.activeClue?.speechText).toBe(
      started.snapshot?.round?.spokenWord,
    );
    ...
    await applyPlayerAction({
      ...
      playerToken: player.playerToken,
      action: {
        actionId: "custom-sentence",
        type: "clue.request",
        roundId: started.snapshot!.round!.roundId,
        clue: "sentence",
      },
    });
    const afterSentence = await readPartySnapshot({
      roomId: room.roomId,
      role: "presenter",
      credential: room.presenterToken,
      lastSequence: 0,
    });
    const expectedSentence = customDeck.words.find(
      ({ word }) => word === started.snapshot?.round?.spokenWord,
    )?.sentence;
  ...
  it("allows a player with the room code to join while the lobby is open", async () => {
    ...
    await applyPresenterAction({
      roomId: room.roomId,
      presenterToken: room.presenterToken,
      action: { actionId: "start-after-code", type: "round.start" },
    });
    expect(
      await joinPartyRoom({ roomId: room.roomId, name: "Daniel", joinId: "late-code-join" }),
    ).toEqual({
      ok: false,
  ...
  it("keeps the word and private drafts secret, then reveals and scores everyone together", async () => {
    ...
    const started = await applyPresenterAction({
      roomId: room.roomId,
      presenterToken: room.presenterToken,
      action: { actionId: "start-1", type: "round.start" },
    });
    ...
    const mayaDraft = await applyPlayerAction({
      roomId: room.roomId,
      playerId: maya.playerId,
      playerToken: maya.playerToken,
      action: {
        actionId: "maya-draft",
        type: "draft.update",
        roundId: countdown.round!.roundId,
        draft: secret.word,
        draftRevision: 1,
      },
    });
    ...
    await applyPlayerAction({
      roomId: room.roomId,
      playerId: daniel.playerId,
      playerToken: daniel.playerToken,
      action: {
        actionId: "daniel-draft",
        type: "draft.update",
        roundId: countdown.round!.roundId,
        draft: "wrong",
        draftRevision: 1,
    ...
    const presenterBeforeReveal = await readPartySnapshot({
      roomId: room.roomId,
      role: "presenter",
      credential: room.presenterToken,
      lastSequence: 0,
    ...
    await applyPlayerAction({
      ...
      playerId: maya.playerId,
      playerToken: maya.playerToken,
      action: { actionId: "maya-lock", type: "answer.lock", roundId: countdown.round!.roundId },
    });
    const allLocked = await applyPlayerAction({
      roomId: room.roomId,
      playerId: daniel.playerId,
      playerToken: daniel.playerToken,
      action: { actionId: "daniel-lock", type: "answer.lock", roundId: countdown.round!.roundId },
    });
    ...
    const revealed = await readPartySnapshot({
      roomId: room.roomId,
      role: "presenter",
      credential: room.presenterToken,
      lastSequence: allLocked.snapshot!.sequence,
    });
  ...
  it("automatically locks the latest synchronized draft and returns a complete reconnect snapshot", async () => {
    ...
    const started = await applyPresenterAction({
      roomId: room.roomId,
      presenterToken: room.presenterToken,
      action: { actionId: "start-auto", type: "round.start" },
    });
    ...
    const reconnect = await readPartySnapshot({
      roomId: room.roomId,
      role: "player",
      credential: player.playerToken,
      playerId: player.playerId,
    ...
    expect(reconnect.snapshot?.player).toMatchObject({
      draft: "latest draft",
      draftRevision: 4,
      locked: true,
      lockedAutomatically: true,
    });
    const late = await applyPlayerAction({
      roomId: room.roomId,
      playerId: player.playerId,
      playerToken: player.playerToken,
      action: { actionId: "late-lock", type: "answer.lock", roundId: round.roundId },
    });
  ...
  it("attributes a shared clue once and makes join and actions idempotent", async () => {
    ...
    const started = await applyPresenterAction({
      roomId: room.roomId,
      presenterToken: room.presenterToken,
      action: { actionId: "same-start", type: "round.start" },
    });
    const repeatedStart = await applyPresenterAction({
      roomId: room.roomId,
      presenterToken: room.presenterToken,
      action: { actionId: "same-start", type: "round.start" },
    });
    ...
    vi.setSystemTime(started.snapshot!.round!.answerOpensAt + 1);
    const clue = await applyPlayerAction({
      roomId: room.roomId,
  ...
  it("closes rooms idempotently and keeps finished results only for the reconnect grace period", async () => {
    ...
    const disposable = await createPartyRoom({
      deckId: "warm-up",
      answerSeconds: 10,
      roundTotal: 1,
    });
    ...
    const finished = await applyPresenterAction({
      roomId: room.roomId,
      presenterToken: room.presenterToken,
