...
export default defineWebSocketHandler({
  async message(peer, message) {
    ...
    if (payload.type === "hello") {
      ...
      peer.subscribe(`spelling-party:${payload.roomId}`);
      peer.send(JSON.stringify({ type: "ready" }));
      peer.publish(`spelling-party:${payload.roomId}`, JSON.stringify({ type: "wake" }));
      return;
    }
    ...
    if (payload.type === "changed") peer.publish(`spelling-party:${session.roomId}`, JSON.stringify({ type: "wake" }));
  },
  close(peer) { const session = sessions.get(peer.id); if (session) peer.unsubscribe(`spelling-party:${session.roomId}`); sessions.delete(peer.id); },
