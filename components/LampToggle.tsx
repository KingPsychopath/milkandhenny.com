...
function isWithinRoute(path: string, route: string) {
...
function resolvedDark(stored: string | null, systemDark: boolean) {
...
export function LampToggle() {
  ...
  /** Follow the system until the user pulls the cord and chooses a mode. */
  useEffect(() => {
    const media = window.matchMedia("(prefers-color-scheme: dark)");
    const apply = () => {
      if (autoForced.current) return;
      const shouldBeDark = resolvedDark(getStored("theme"), media.matches);
  ...
  useEffect(() => {
    ...
    if (!isPhotoPage && autoForced.current) {
      ...
      const original = savedPref.current ?? "system";
      const shouldBeDark = resolvedDark(
        original,
