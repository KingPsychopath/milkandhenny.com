...
import { renderCommunicationMessage, type CommunicationMedia } from "@/features/communications/email.server";
...
async function handleGET(request: Request) {
  ...
  try {
    ...
    return Response.json({ contacts, messages, events, plans, templates, surveys });
  } catch (error) {
...
async function handlePOST(request: Request) {
  ...
  try {
    ...
    if (body.action === "preview") {
      ...
      if (kind !== "newsletter" && kind !== "event_update" && kind !== "pitch_nudge" && kind !== "event_service" && kind !== "feedback") {
      ...
      if (!subject || !messageBody) return Response.json({ error: "Add a subject and message first" }, { status: 400 });
      const event = typeof body.eventSlug === "string" && body.eventSlug ? ((await getEvent(body.eventSlug)) ?? undefined) : undefined;
    ...
    if (body.action === "create-starter-plan") {
      if (typeof body.eventSlug !== "string" || !body.eventSlug) return Response.json({ error: "Choose an event" }, { status: 400 });
      return Response.json({ plan: await createStarterPlan(body.eventSlug) });
    }
    if (body.action === "schedule-plan" || body.action === "pause-plan") {
      if (typeof body.planId !== "string" || !body.planId) return Response.json({ error: "Choose a plan" }, { status: 400 });
    ...
    if (body.action === "update-stage") {
      if (typeof body.stageId !== "string" || typeof body.subject !== "string" || typeof body.messageBody !== "string") {
    ...
    if (body.action === "save-template") {
      if (typeof body.name !== "string" || typeof body.kind !== "string" || typeof body.subject !== "string" || typeof body.messageBody !== "string") {
        return Response.json({ error: "The template needs a name, type, subject, and message" }, { status: 400 });
      }
      const kind = body.kind;
      if (!["newsletter", "event_update", "pitch_nudge", "event_service", "feedback"].includes(kind)) {
        return Response.json({ error: "Choose a valid template type" }, { status: 400 });
      }
      return Response.json({ template: await saveCommunicationTemplate({
        id: typeof body.templateId === "string" ? body.templateId : undefined,
        name: body.name,
        kind: kind as "newsletter" | "event_update" | "pitch_nudge" | "event_service" | "feedback",
        subject: body.subject,
        body: body.messageBody,
        media: body.media,
    ...
    if (body.action === "archive-template") {
      if (typeof body.templateId !== "string" || !body.templateId) return Response.json({ error: "Choose a template" }, { status: 400 });
