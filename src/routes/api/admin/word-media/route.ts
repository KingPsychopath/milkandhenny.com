...
async function handleGET(request: Request) {
  ...
  try {
    ...
    const includeAssets = new URL(request.url).searchParams.get("includeAssets") !== "false";
    ...
    return apiErrorFromRequest(
      request,
      "admin.word-media",
      "Failed to load word media library",
      error,
    );
  }
}

export const Route = createFileRoute("/api/admin/word-media")({
  server: {
    handlers: {
