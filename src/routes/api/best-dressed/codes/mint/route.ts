...
async function handlePOST(request: Request) {
  ...
  try {
    ...
    for (let i = 0; i < maxAttempts; i++) {
      ...
      return Response.json({ success: true, code, ttlSeconds, expiresAt });
    }

    return Response.json({ error: "Failed to mint code (try again)" }, { status: 503 });
  } catch (error) {
    return apiErrorFromRequest(
      request,
      "best-dressed.codes.mint",
      "Failed to mint vote code",
      error,
    );
  }
}

export const Route = createFileRoute("/api/best-dressed/codes/mint")({
  server: {
    handlers: {
