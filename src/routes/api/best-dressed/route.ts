...
import { getClientIp, requireAdminStepUp, requireAuth } from "@/features/auth/auth.server";
...
function getOrCreateVoterId(request: Request): { voterId: string; isNew: boolean } {
  const existing = getCookie(request, VOTE_COOKIE);
...
function attachVoterCookie(res: Response, voterId: string): void {
  setCookie(res, VOTE_COOKIE, voterId, {
    httpOnly: true,
    sameSite: "lax",
